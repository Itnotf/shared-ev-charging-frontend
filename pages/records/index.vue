<template>
      ...
      <template v-if="shiftPie && shiftPie.totalKwh > 0">
        <view class="shift-pie-flex compact">
          ...
          <view class="shift-pie-legend right-legend">
            <view class="legend-item"><span class="legend-dot day"></span> {{ TIMESLOTS.day.name }} {{ shiftPie ? shiftPie.dayKwh || 0 : 0 }} kWh</view>
            <view class="legend-item"><span class="legend-dot night"></span> {{ TIMESLOTS.night.name }} {{ shiftPie ? shiftPie.nightKwh || 0 : 0 }} kWh</view>
          </view>
        </view>
      </template>
      <template v-else>
    ...
    <view class="chart-card">
      ...
      <view v-if="dailyKwh.length > 0" class="chart-bar-group" ref="barGroup" style="position:relative;">
        <!-- tooltip -->
        <view v-if="tooltip.show" class="chart-tooltip" :style="{left: tooltip.left + 'px'}">
          <text>{{ tooltip.date }}</text>
          <text>{{ tooltip.kwh }} kWh</text>
					</view>
        <view v-for="(item, index) in dailyKwh" :key="item.date" class="chart-bar-item"
          :style="{width: chartBarWidth + 'px', marginRight: (index < dailyKwh.length - 1 ? chartBarGap + 'px' : '0')}"
          @touchstart="showTooltip($event, item, index)" @touchend="hideTooltip" @mouseleave="hideTooltip">
          <view class="chart-bar" :style="{width: chartBarWidth + 'px', height: getBarHeight(item.totalKwh)}"></view>
          <text v-if="shouldShowLabel(index, dailyKwh.length)" class="chart-bar-label">{{ item.date.slice(5, 10) }}</text>
							</view>
						</view>
...
export default {
  components: { CommonNavBar, SvgIcon, qiunDataCharts, uniIcons },
	data() {
		return {
      ...
      pieOpts: {
        color: ['#FFA500', '#409EFF'],
        legend: { show: false },
        extra: { pie: { labelWidth: 30, border: false, activeOpacity: 0.5, activeRadius: 10, customRadius: 45 } } // customRadius≈90rpx，留更多空白
      },
  ...
  mounted() {
    ...
    async loadDailyStatistics() {
      ...
      const dateMap = {};
      rawList.forEach(item => { dateMap[item.date] = item; });
      const fullList = [];
    ...
    async loadMonthlyShift() {
      ...
      this.shiftPie = res && res.data ? res.data : null;
		},
    getBarHeight(kwh) {
      // 简单映射高度，最大高度120rpx
